...
#let project(
  ...
  set text(font: ("New Computer Modern","Georgia", "SimSun"), size: 12pt, lang: "zh")
  show raw: set text(font: ("Consolas","JetBrains Mono", "LXGW Wenkai"), size: 9pt, lang: "zh")
  ...
  if (theme == "lab") {
    v(1fr)
    {
      set align(center)
              // set align(center)
            line(length: 100%)
        par(text(size: 2em, weight: 900, fakebold[信号与系统 · 课程设计]))
            v(0.5fr)
        line(length: 100%)
        v(0.5fr)
        par(text(size: 2.5em, weight: 700, fakebold[快速傅里叶变换的实现和优化]))
            v(2fr)
        // block(width: 70%, asset-zju-banner)
...
// 在文档开头或者生成目录之前，添加这条规则
  show outline.entry: it => {
    // 使用 block 来为每个目录条目添加下方的间距
    // 调整 0.65em 这个值来改变行距大小
      block(it, spacing: 1.2em)
  }
